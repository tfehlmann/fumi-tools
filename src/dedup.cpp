...
struct custom_bam1_hash {
    ...
    std::size_t operator()(const bam1_t* lhs) const {
        ...
        return robin_hood::hash_int(lhs->core.pos) ^ robin_hood::hash_int(lhs->core.tid) ^ robin_hood::hash_bytes(bam_get_qname(lhs), std::strlen(bam_get_qname(lhs)))
                ^ robin_hood::hash_int(lhs->core.mpos) ^ robin_hood::hash_int(lhs->core.mtid) ^ robin_hood::hash_int(lhs->core.isize)
...
struct custom_bam1_eq {
    template<typename deleter>
    bool operator()(const std::unique_ptr<bam1_t, deleter>& lhs, const std::unique_ptr<bam1_t, deleter>& rhs) const{
        return this->operator()(lhs.get(), rhs.get());
    }

    bool operator()(const bam1_t* lhs, const bam1_t* rhs) const{
        auto check1 = lhs->core.pos == rhs->core.pos && lhs->core.tid == rhs->core.tid
                && lhs->core.mpos == rhs->core.mpos && lhs->core.mtid == rhs->core.mtid
                && lhs->core.isize == rhs->core.isize;
        if(!check1 || nonstd::string_view(bam_get_qname(lhs)) != nonstd::string_view(bam_get_qname(rhs))){
            return false;
        } else {
            auto hi_lhs = bam_aux_get(lhs, "HI");
            bool same_hi = true;
...
void update_read_map(
    bam1_t* read, uint64_t pos, ReadGroup key, std::string umi,
    ...
    robin_hood::unordered_flat_set<std::unique_ptr<bam1_t, bam1_t_deleter>,
                ...
                custom_bam1_eq
                >& paired_read_map,
        robin_hood::unordered_set<bam1_t*, custom_bam1_hash, custom_bam1_eq>& current_reads) {
  ...
  if (it != std::end(read_map[pos][key])) {
    ...
    if (read_qual < other_qual) {
        ...
        if(read->core.mtid < read->core.tid || (read->core.mtid == read->core.tid && read->core.isize <= 0)){
    ...
    if (read_qual > other_qual) {
      ...
      if(res.first->core.mtid < res.first->core.tid || (res.first->core.mtid == res.first->core.tid && res.first->core.isize <= 0)){
...
template<class ReadGroup>
std::ostream& operator<<(std::ostream& out, const robin_hood::unordered_flat_map<
                         ReadGroup,
                         robin_hood::unordered_flat_map<
                             std::string,
                         std::pair<std::unique_ptr<bam1_t, bam1_t_deleter>, uint64_t>>>& lhs){
    out << '{';
    for(auto& e: lhs){
        out << e.first << ':';
...
std::ostream& operator<<(std::ostream& out, const robin_hood::unordered_flat_set<std::unique_ptr<bam1_t, bam1_t_deleter>,
                         custom_bam1_hash,
                         custom_bam1_eq
                         >& lhs){
    out << '{';
...
template <class ReadGroup, class Fun>
void process_bam_read_chunks_helper(samFile* file, bam_hdr_t* bam_hdr, umi_opts opts,
                                    samFile* out, Fun fun) {
    auto cur_ref = 0;
    auto last_ref = -1;
    auto last_pos = 0ul;
    ...
    auto output_positions = [&read_map, &read_counts,
                             &fun, &current_reads,
            &paired_read_map, &not_yet_paired_reads](nonstd::optional<uint64_t> start, int32_t bam_pos) {
    ...
    while (sam_read1(file, bam_hdr, record) > 0) {
      if ((record->core.flag & BAM_FUNMAP) == 0) {
        ...
        if((record->core.flag & BAM_FPAIRED) != 0 && record->core.tid != record->core.mtid){
            // chimeric read pair
        ...
        update_read_map(record, pos, key, std::string{umi}, read_map,
                        read_counts, paired_read_map, current_reads);
      }
      progress.update();
    }
    output_positions(nonstd::nullopt, std::numeric_limits<int32_t>::max());
...
void process_bam_read_chunks(samFile* file, bam_hdr_t* bam_hdr, umi_opts opts,
                             samFile* out, Fun fun) {
    if(opts.paired){
        process_bam_read_chunks_helper<read_group_paired>(file, bam_hdr, opts, out, fun);
    } else {
        process_bam_read_chunks_helper<read_group>(file, bam_hdr, opts, out, fun);
    }
}

template <class Fun>
void process_paired_reads(std::unique_ptr<bam1_t, bam1_t_deleter>& r1,
                          robin_hood::unordered_flat_set<std::unique_ptr<bam1_t, bam1_t_deleter>,
                          custom_bam1_hash,
                          custom_bam1_eq
                          >& paired_reads,
    ...
    if((r1->core.flag & BAM_FMUNMAP) == 0) { // has mate
        if(r1->core.mpos <= bam_pos){ // we can only have the mate if it was before our current position
...
void dedup(const std::string& input, const std::string& output, umi_opts opts) {
  ...
  samFile* out = hts_open(output.c_str(), opts.uncompressed ? "wbu": ends_with(output, ".bam") ? "wb" : "w");
  if (out == nullptr) {
  ...
  process_bam_read_chunks(
      file, bam_hdr, opts, out, [&clusterer, &out, &bam_hdr](auto& bundle, auto bam_pos, auto& paired_read_map, auto& not_yet_paired_reads) {
        clusterer(bundle, [&out, &bam_hdr, &paired_read_map, &not_yet_paired_reads, &bam_pos](auto& read, auto& /*umi*/, auto& /*count*/) {
            process_paired_reads(read, paired_read_map, not_yet_paired_reads, bam_pos,
